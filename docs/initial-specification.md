...
ノードはノードパレットからキャンバスへドラッグ＆ドロップして配置します。パレット項目を選択しただけの状態でキャンバスをクリックしても、新しいノードは作成しません。キャンバス上の既存ノードはドラッグで移動します。

ノードパレットは、各Blockの`NodeType`でグループ化します。初期グループは`INPUT`、`Basic Process`、`OUTPUT`の三種類とし、将来のBlock追加に合わせて種類を増やせるようにします。Blockの用途説明や互換性のための既存カテゴリ文字列は保持できますが、パレットの主要な整理軸には使用しません。
...
キャンバスに置かれたBlockは、共通設定として名前、説明、有効を持ちます。有効が`false`の場合、そのBlockは実行されず、データも出力しません。Block固有設定は各Blockの仕様で定義します。Block定義はタイトル、サブタイトル、アイコンを持てます。タイトルとサブタイトルはノードライブラリで表示し、キャンバス上では配置済みBlockの名前のみを表示します。名前が未定義の場合はBlockタイトルを表示します。BlockアイコンはBlockフォルダの`icon.png`を使用し、64 x 64 pxを推奨します。
...
| フロー制約 | MVPはDAG、循環禁止 |
| ブロック間通信 | Fast StreamとPayload（内部契約名: JSON Message）の二系統 |
