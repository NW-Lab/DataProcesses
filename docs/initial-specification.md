...
| ノード | 区分 | 入力 | 出力 | 主要設定 |
|---|---|---|---|---|
| Test Signal | Source | なし | `FastStreamFrame<TimeSeries>` | 波形、周波数、振幅、サンプルレート、チャンネル数、ブロック長 |
| Filter | Processing | `FastStreamFrame<TimeSeries>` | `FastStreamFrame<TimeSeries>` | Low-pass／High-pass、カットオフ、次数 |
| FFT | Analysis | `FastStreamFrame<TimeSeries>` | `SpectrumFrame` | FFTサイズ、窓関数、オーバーラップ、振幅表現 |
| Time Series | Visualization | `FastStreamFrame<TimeSeries>` | 任意のパススルー | 表示時間幅、Y軸、色、更新頻度 |
| Python Output | Output | `FastStreamFrame`、`SpectrumFrame`またはJSON Message | JSON Messageによる状態／任意の応答 | 実行環境、スクリプト、タイムアウト、送信方式 |

Test Signalは少なくとも正弦波とホワイトノイズに対応します。FilterはMVPではLow-passとHigh-passに限定します。FFTは実数入力を対象にし、周波数軸と振幅値を返します。Time Seriesは描画負荷を制御するため、処理サンプル全件ではなく表示用に間引けるようにします。
...
FastStreamFrame
  ...
  StreamId: string
  SequenceNumber: long
  StartTimeUnixNs: long
  SamplePeriodNs: long
  ChannelNames: string[]
  Unit: string[]
  Samples: double[channel][sample]
  QualityFlags: optional byte[channel][sample]
  Metadata: dictionary<string, scalar>
```

`StartTimeUnixNs`は基準時刻、`SamplePeriodNs`はサンプル間隔です。ナノ秒単位の整数を採用するのは、1 ms未満の間隔を表現できるようにするためであり、計測精度が必ずナノ秒になることを意味しません。実際の時刻精度は入力デバイスとOSタイマーに依存します。

不規則サンプリング、欠測、外部イベント時刻を扱う場合は、次の任意フィールドを追加します。

```text
  TimeOffsetNs: optional long[sample]
```

`TimeOffsetNs`が存在する場合、各時刻は `StartTimeUnixNs + TimeOffsetNs[i]` とします。規則的なデータでは省略します。
...
`payload`はJSONのobject、array、string、number、boolean、nullを許可します。`schemaVersion`、`messageId`、`timestamp`、`topic`を共通項目とし、ブロック固有の内容は`payload`へ格納します。秘密情報や巨大なバイナリデータをJSONへ直接埋め込むことは避けます。
...
SpectrumFrame
  ...
  FrequencyStartHz: double
  FrequencyStepHz: double
  ChannelNames: string[]
  Values: double[channel][bin]
  Unit: string[]
  Metadata: dictionary<string, scalar>
```

ポート外観はFast Streamと同じ青系・円形とし、ツールチップとポートラベルで `TimeSeries` と `Spectrum` を区別します。TimeSeries入力へSpectrumを直接つなぐなど、詳細スキーマが異なる接続は拒否します。
...
C#ノードは、公開SDKのインターフェースを実装した.NETアセンブリとして提供します。.NETでは、`AssemblyLoadContext`と`AssemblyDependencyResolver`を使い、プラグインごとに依存関係を分けて読み込む公式例があります。[3] 一方、Microsoftの資料も、信頼できないコードを同一プロセスへ安全に読み込むことはできないと注意しています。[3]

このため、プラグインを二段階で設計します。

| 種別 | 実行場所 | 用途 | 導入時期 |
|---|---|---|---|
| Managed Node Plugin | アプリと同一.NETプロセス | 公式・信頼済みC#ノード、高速処理 | MVP |
| Worker Plugin | 別プロセス | Python、他言語、未信頼プラグイン、障害分離 | v0.2以降 |

MVPの公開契約は、概念的に次の要素を含みます。
...
プラグインには `plugin.json` を同梱し、ID、名称、バージョン、対応するSDKバージョン、エントリアセンブリ、提供ノード一覧、作者、ライセンスを記録します。ノード型IDは衝突を避けるため、`org.example.package.node-name` のような逆ドメイン形式を推奨します。
