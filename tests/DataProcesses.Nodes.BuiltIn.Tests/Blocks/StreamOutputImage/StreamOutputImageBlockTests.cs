...
namespace DataProcesses.Nodes.BuiltIn.Tests.Blocks.StreamOutputImage;

public sealed class StreamOutputImageBlockTests
{
    [Fact]
    public void Definition_UsesOneImageInput()
    {
        var port = Assert.Single(StreamOutputImageBlock.Definition.Ports);

        Assert.Equal(StreamOutputImageBlock.InputPortId, port.Id);
    ...
    {
        var node = new StreamOutputImageNode();
        ...
        await node.OnPacketAsync(StreamOutputImageBlock.InputPortId, image, CancellationToken.None);

        var snapshot = Assert.IsType<StreamOutputImageSnapshot>(node.LatestSnapshot);
    ...
    public async Task OnPacketAsync_TruncatesPreviewWhenImageExceedsMaximum()
    {
        var node = new StreamOutputImageNode();
        await node.InitializeAsync(new RecordingNodeContext(), CancellationToken.None);
        var pixels = new byte[StreamOutputImageNode.MaximumPreviewBytes + 32];
        var image = new ImageFrame(
            name: "camera-large",
            width: pixels.Length,
            height: 1,
        ...
        var snapshot = Assert.IsType<StreamOutputImageSnapshot>(node.LatestSnapshot);
        Assert.Equal(StreamOutputImageNode.MaximumPreviewBytes, snapshot.PreviewPixels.Length);
    }
    ...
    public async Task OnPacketAsync_RejectsNonImagePackets()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(async () =>
            await node.OnPacketAsync(StreamOutputImageBlock.InputPortId, vector, CancellationToken.None));
    }
}
