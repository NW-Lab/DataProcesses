...
/// </summary>
public sealed class NumericVectorOutputNode : INode
{
    ...
    public NumericVectorOutputSnapshot? LatestSnapshot { get; private set; }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.Equals(inputPortId, NumericVectorOutputBlock.InputPortId, StringComparison.Ordinal))
        {
        ...
        {
            throw new ArgumentException("Numeric Vector Output accepts Numeric Vector input only.", nameof(packet));
        }

        var values = Downsample(vector.Values);
        LatestSnapshot = new NumericVectorOutputSnapshot(
            vector.Name,
            values,
            vector.Length,
